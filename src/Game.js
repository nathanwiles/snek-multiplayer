...
class Game {
  ...
  messageAllButOne(message, client) {
    for (const snake of this.snakes) {
      if (snake.client !== client) {
        snake.client.write(message);
      }
    }
  }
  ...
  handlePlayerInput(input, client) {
    ...
    if (dir) {
      ...
      // Written by Nathan Wiles
      let newName = name[1].trim().substring(0, MAX_PLAYER_NAME_LENGTH);
      // let player know they're in and other players know they're in
      try {
        this.messageClient(`You are now known as ${newName}\n`, client);
        this.messageAllButOne(`${newName} joined the game\n`, client);
      } catch (e) {
        console.log("error writing to client: ", e);
      }
      // End of Nathan Wiles' code


      return snake.changeName(newName);
    } else if (say) {
      return snake.setMessage(
        say[1].trim().substring(0, MAX_PLAYER_MSG_LENGTH)
      );
    }
  ...
  safeSnakeStartingCoords() {
    ...
    while (attempts < 100) {
      ...
      const x = randomNum(
        0 + INITIAL_SNAKE_SIZE,
        this.ui.gameContainer.width - 1 - INITIAL_SNAKE_SIZE
      );
      const y = randomNum(
        0 + INITIAL_SNAKE_SIZE,
        this.ui.gameContainer.height - 1 - INITIAL_SNAKE_SIZE
      );
  ...
  playerLeft(client) {
    ...
    const thisSnake = this.snakes[index]; // Written by Nathan Wiles

    if (index >= 0) {
      this.messageAllButOne(`${thisSnake.name} left the game\n`, client); // Written by Nathan Wiles
      this.removeSnake(thisSnake, index);
    }
  ...
  drawSnake(snake) {
    ...
    if (snake.message) {
      const msg = snake.name
        ? `${snake.name}: ${snake.message}`
        : snake.message;
